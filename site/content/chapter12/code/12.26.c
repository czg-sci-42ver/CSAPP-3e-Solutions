...
struct hostent *gethostbyname_ts(const char *name, struct hostent *host) {
  ...
  host->h_aliases = (char **)Malloc(sizeof(char *) * (i));
  for (i = 0; sharehost->h_aliases[i] != NULL; i++) {
    // copy every char *
    host->h_aliases[i] = (char *)Malloc(strlen(sharehost->h_aliases[i]) + 1);
  ...
  for (i = 0; sharehost->h_addr_list[i] != NULL; i++) {
    // copy every char *
    host->h_addr_list[i] =
        (char *)Malloc(strlen(sharehost->h_addr_list[i]) + 1);
...
int main(int argc, char *argv[]) {
  ...
  struct hostent host;
  gethostbyname_ts("www.baidu.com", &host);
  // result in &host

  printf("h_name: %s;\n", host.h_name);
  for (int i = 0; host.h_aliases[i] != NULL; i++) {
    printf("%d: %s; %s\n", i, host.h_aliases[i], host.h_addr_list[i]);
  }
