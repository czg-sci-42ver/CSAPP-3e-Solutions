...
void *loop_thread(void *argp);

struct thread_arg {
  int connfd, logfd, thread_index;
  char **block_list;
  sem_t mutex;
};

sem_t unused_thread_mutex;
...
int main(int argc, char **argv) {
  ...
  int logfd, use_thread = 0;
  struct thread_arg threads_arg[THREADS];
  pthread_t tid;
  ...
  Sem_init(&unused_thread_mutex, 0, 1);
  ...
  for (i = 0; i < THREADS; i++) {
    ...
    threads_arg[i] = (struct thread_arg){
        .thread_index = i,
        .block_list = (char **)block_list,
        .logfd=logfd,
    };
    Sem_init(&threads_arg[i].mutex, 0, 1);
  }
  while (1) {
  ...
  Check_thread:
    ...
    threads_arg[use_thread].connfd=connfd;
    Pthread_create(&tid, NULL, loop_thread, &threads_arg[use_thread]);
  }
...
void *loop_thread(void *argp) {
  ...
  int clientfd;
  ssize_t sn, cn;
  Rio_readinitb(&server_rio, arg.connfd);
  /*
  ...
  printf("sbuf: %s\n", s_buf);
  // if uri is blocked?
  if (blocked_uri(uri, (char(*)[MAXLINE])arg.block_list)) {
    printf("%s is blocked\n", uri);
    Close(arg.connfd);
    return NULL;
  }
  ...
  Write(arg.logfd, log_buf, strlen(log_buf));
  ...
  while ((cn = Rio_readlineb(&client_rio, c_buf, MAXLINE)) != 0)
    Rio_writen(arg.connfd, c_buf, cn);
